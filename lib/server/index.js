...
module.exports.listen = (port) => {
    ...
    server.on('connection', (client) => {
        socket.init(client, `[@s][client][${client.remoteAddress}:${client.remotePort}]`, logger);
    ...
    const listen = () => {
        server.listen(port, () => {
            logger.success(`[@s][server] listening on port ${port}`);
        });
    };

    server.on('error', (e) => {
        if (e.code === 'EADDRINUSE') {
            logger.error('[@s][server] listen address in use, retrying...');
            setTimeout(() => {
                server.close();
                listen();
            }, 2000);
        } else {
            logger.error(`[@s][server] error occurred ${e.message}`);
            server.close();
        }
    });
