    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            client.file.stream.on('close', function () {
                console.log(`${client.tag} write file stream closed: ${client.file.path}`);
            });
    ...
    return new Promise((resolve, reject) => {
        ...
        client.file.stream.on('readable', function () {
            ...
            while (null !== (chunk = client.file.stream.read(4096))) {
                try {
                    client.write(packet.pack(client, packet.type.DATA, chunk.toString("base64"), true));
                } catch (err) {
                    reject(err);
                    break;
                }
        ...
        client.file.stream.on('close', function () {
            console.log(`${client.tag} read file stream closed`);
            resolve();
        });
